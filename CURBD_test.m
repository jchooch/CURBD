...
for ensemble_index = 1:1
    ...
    for iTarget = 1:size(test_CURBD,1)
        ...
        for iSource = 1:size(test_CURBD,2)
            ...
            for neuron = 1:length(test_regions{iTarget, 2})
                summed_correlations = summed_correlations + corrcoef(test_CURBD{iTarget, iSource}(neuron, :), target_activities(neuron, :));
            end
            fprintf('correlation sum: %d \n', summed_correlations);
    ...
    saveas(gcf, sprintf('CURBD_fig_%d.png', ensemble_index));
